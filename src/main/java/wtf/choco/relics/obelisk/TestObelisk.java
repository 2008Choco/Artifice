package wtf.choco.relics.obelisk;
...
public class TestObelisk extends Obelisk {
    ...
    private static final ObeliskStructure STRUCTURE = ObeliskStructure.withSize(1, 3, 3)
            .set(0, 0, 1, Material.OAK_LOG).set(0, 1, 0, Material.OAK_FENCE)
            .set(0, 1, 1, Material.OAK_LOG).set(0, 1, 2, Material.OAK_FENCE)
            .set(0, 2, 0, Material.OAK_PLANKS).set(0, 2, 1, Material.OAK_LOG).set(0, 2, 2, Material.OAK_PLANKS)
            .formationPoint(0, 0, 1);
    ...
    public TestObelisk(Relics plugin) {
        super(new NamespacedKey(plugin, "test"), STRUCTURE);
    }
